...
export const readDevinQuota = (payload: unknown) => {
  ...
  const userStatus =
    isRecord(payload) && isRecord(payload.userStatus) ? payload.userStatus : {}
  ...
  const observed = windows.some(
    (window) => window.remainingPercent !== null || window.resetAtMs !== undefined,
  )
  ...
  return {
    ...
    plan:
      typeof planInfo.planName === "string"
        ? planInfo.planName.trim() || undefined
        : undefined,
    subscriptionActiveUntil:
      typeof planEnd === "string" &&
      /^\d{4}-\d{2}-\d{2}T/.test(planEnd) &&
      Number.isFinite(Date.parse(planEnd))
        ? planEnd
        : undefined,
  }
}
