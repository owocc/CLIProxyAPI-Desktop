...
export const authFileName = (file: AuthFileRecord): string =>
  readString(file, "name") || "未命名凭据"

export const isRuntimeOnlyAuthFile = (file: AuthFileRecord): boolean =>
...
export const normalizeOAuthProvider = (value: string): string => {
...
export const authFileProvider = (file: AuthFileRecord): string =>
...
export const snapshotAuthFiles = (files: AuthFileRecord[]): AuthFileSnapshot => {
  ...
  return new Map(
    Array.from(grouped, ([name, fingerprints]) => [
      name,
...
export const authFileTimestamp = (file: AuthFileRecord): number => {
...
const authFilePriority = (file: AuthFileRecord): number => {
  let score = 0
...
const mergeDuplicateAuthFiles = (entries: AuthFileRecord[]): AuthFileRecord => {
  const sorted = [...entries].sort((left, right) => {
    ...
    return (
      Object.values(right).filter(hasMeaningfulValue).length -
...
export const dedupeAuthFiles = (files: AuthFileRecord[]): AuthFileRecord[] => {
  ...
  return Array.from(grouped.values())
    ...
    .sort((left, right) =>
      authFileName(left).localeCompare(authFileName(right), undefined, {
        sensitivity: "base",
      }),
    )
}
