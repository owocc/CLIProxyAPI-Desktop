import { ProviderHealthProbe } from "../../bindings/easycliproxyapi/internal/service/providerservice"
...
export function buildProviderHealthProbe(
  ...
  const normalizedModel =
    provider === "gemini" ? model.trim().replace(/^models\//i, "") : model.trim()
  const metadata = {
  ...
  if (provider === "claude") {
    const bearerToken =
      headerValue(headers, "authorization").match(/^Bearer\s+(.+)$/i)?.[1]?.trim() ?? ""
...
export async function checkProviderHealthProbe(
  ...
  try {
    ...
    const response = await ProviderHealthProbe({
    ...
    const firstTokenLatencyMs =
      response.firstTokenLatencyMs != null && Number.isFinite(response.firstTokenLatencyMs)
        ? Math.max(1, Math.round(response.firstTokenLatencyMs))
        : undefined
    const responseLatencyMs =
      response.responseLatencyMs != null && Number.isFinite(response.responseLatencyMs)
...
export async function checkProviderModelsHealth(
  ...
  const worker = async () => {
    while (currentIndex < models.length) {
      ...
      if (!model) continue
      const result = await checkProviderModelHealth(options, model.name)
