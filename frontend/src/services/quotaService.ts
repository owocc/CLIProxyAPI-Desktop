...
import { getCurrentLocale, translate, type AppLocale } from './quotaI18n';
import { updateQuotaCache } from './quotaCache';
...
export function loadQuota(file: AuthFile): Promise<QuotaState> {
  ...
  if (reset) {
    return reset.catch((error): QuotaState => {
      const errState: QuotaState = {
        status: 'error',
        rows: [],
        error: error instanceof Error ? error.message : String(error),
      };
      updateQuotaCache((cur) => ({ ...cur, [key]: errState }));
      return errState;
    });
  }
  ...
  // Immediately set cache status to loading
  updateQuotaCache((current) => ({
    ...current,
    [key]: { status: 'loading', rows: current[key]?.rows || [] },
  }));

  const request = loadQuotaSnapshot(file)
    .then((result) => {
      updateQuotaCache((current) => ({ ...current, [key]: result }));
      return result;
    })
    .catch((error) => {
      const errorState: QuotaState = {
        status: 'error',
        rows: [],
        error: error instanceof Error ? error.message : String(error),
      };
      updateQuotaCache((current) => ({ ...current, [key]: errorState }));
      return errorState;
    })
    .finally(() => {
...
function runQuotaMutation(file: AuthFile, mutate: () => Promise<QuotaState>): Promise<QuotaState> {
  ...
  const request = (async () => {
    ...
    updateQuotaCache((current) => ({ ...current, [key]: res }));
    return res;
  })().finally(() => {
