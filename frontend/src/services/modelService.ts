...
export function usableModelAlias(value: string | undefined | null): string {
  ...
  for (let i = 0; i < alias.length; i++) {
    ...
    if (alias[i].trim() === "" || code < 32 || code === 127) return ""
  }
  return alias
}
...
export function normalizeBaseUrl(value: string): string {
  ...
  try {
    ...
    throw new Error("Base URL 地址无效")
  }
  if (!["http:", "https:"].includes(parsed.protocol) || !parsed.hostname) {
    throw new Error("不支持的协议或无效主机名")
  }
  parsed.hash = ""
  parsed.search = ""
  return parsed
    ...
    .map((item): ModelOption | null => {
      const name =
        typeof item === "string"
          ? item
          : isRecord(item)
            ? readString(item, "id", "name", "model", "value")
            : ""
      ...
      const displayName = record
        ? readString(record, "display-name", "display_name", "displayName")
        : ""
      const thinking =
        record && isRecord(record.thinking) ? { ...record.thinking } : undefined
      return {
        name,
...
export async function fetchModels(
  ...
  if (candidates.length === 0) throw new Error("缺少 Base URL 地址")
  ...
  if (provider === "gemini") {
    ...
    const bearerToken =
      headerValue("authorization").match(/^Bearer\s+(.+)$/i)?.[1]?.trim() ?? ""
  ...
  for (const url of candidates) {
    try {
      ...
      for (let page = 0; page < (provider === "gemini" ? 20 : 1); page += 1) {
        ...
        const response = await managementApi.post<Record<string, unknown>>(
          "/api-call",
          {
  ...
  throw new Error(lastError || "上游服务未返回可识别的模型列表")
}
